...
|----------------|-----------------|--------------|---------------|-------|
| `502389919`    | individual      | "3-month"    | ~$4,548       | **the options book lives here** |
| `873870497`    | individual      | "9-month"    | ~$7,892       | |
| `710276346`    | ira_roth        | Roth         | ~$32,148      | 46 positions, bulk of the portfolio |
| `929809911`    | individual      | —            | $0            | empty |
| `732997523`    | individual      | —            | $0            | empty |

Plus crypto (Nummus, enrolled, holdings $0) and futures (ceres, active, no open contracts).
Total equity ≈ $44.6K. See §4 for how to enumerate these — the default endpoint hides most of them.
...
```bash
node cli/dist/index.js brokerage execute "portfolios/{num}/" --param num=502389919 --json --full
node cli/dist/index.js brokerage execute "positions/?account_number={account_number}&nonzero=true" --param account_number=502389919 --json --full
```
...
node cli/dist/index.js brokerage execute "https://api.robinhood.com/orders/" --method POST \
  --body-json '{"account":"https://api.robinhood.com/accounts/502389919/","instrument":"https://api.robinhood.com/instruments/<id>/","symbol":"F","type":"limit","time_in_force":"gfd","trigger":"immediate","price":"9.00","quantity":"1","side":"buy"}'
...
node cli/dist/index.js brokerage execute "https://api.robinhood.com/options/orders/" --method POST \
  --body-json "{\"account\":\"https://api.robinhood.com/accounts/502389919/\",\"direction\":\"debit\",\"legs\":[{\"side\":\"buy\",\"option\":\"https://api.robinhood.com/options/instruments/8d6d6f2b-d51c-41e6-8a6f-e0ec7fbed190/\",\"position_effect\":\"open\",\"ratio_quantity\":1}],\"type\":\"limit\",\"time_in_force\":\"gtc\",\"trigger\":\"immediate\",\"price\":\"0.01\",\"quantity\":\"1\",\"ref_id\":\"$REF\"}" \
  --json --full
