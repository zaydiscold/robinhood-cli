...
| Auth | Browser-session bearer token loaded from local `.env`, with one-shot self-heal on `401`. Token injection scripts ([`inject-token.py`](./inject-token.py) / [`inject.py`](./inject.py)) handle the frostbyte→mothership pipeline. |
